...
function downImg(imgurl) {
	var solveUrl = "https:" + imgurl;
	var narr = solveUrl.replace("//image.haha.mx/", "").split("/")
	// 做一步优化，如果存在文件，则不下载
	var filename = "./upload/topic1/" + narr[0]  + narr[1] + narr[2] + "_" + narr[4];
	fs.exists(filename, function(b){
		if (!b) {
			// 文件不存则进行 下载
			http.get(solveUrl.replace("/small/", "/big/"), function(res) {
				var imgData = "";
				...
				res.on("end", function() {
					var savePath = "./upload/topic1/" + narr[1] + narr[2] + "_" + narr[4];
